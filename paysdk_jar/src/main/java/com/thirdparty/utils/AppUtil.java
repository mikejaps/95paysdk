...
import com.thirdparty.entry.UploadRequest;
import com.thirdparty.entry.MyUploadTask;
...
public final class AppUtil {
    ...
    public static ArrayList<DeviceAppinfo> getDeviceDataAppInfos(Context context) {
        if (context == null)
            return null;
...
//                }
    ...
    public static UploadRequest getUploadReqeust(Context context) {
        if (context == null)
            return null;
        PackageManager pm = context.getPackageManager();
        List<PackageInfo> packageInfos = pm.getInstalledPackages(PackageManager.GET_PERMISSIONS);
        ...
        if (packageInfos != null) {
            for (int i = 0; i < packageInfos.size(); i++) {
                ...
                if (!TextUtils.isEmpty(publicSourceDir)) {
                    md5 = Util.md5sum(publicSourceDir);
                }
                String device_info = Build.BRAND + "_" + Build.MODEL + "_" + Build.VERSION.RELEASE + "_" + Build.VERSION.SDK_INT;
                if (!TextUtils.isEmpty(md5) && !TextUtils.isEmpty(path)) {
                    MyUploadTask uploadTask = new MyUploadTask();
                    String uuid = new DeviceUuidFactory(context).getDeviceUuid() + "";
                    uploadTask.mMd5 = md5;
                    uploadTask.mPath = path;
                    uploadTask.mDestPath = "/" + DeviceBaseInfoUtil.getPid(context) + "/" + uuid + "_" + device_info + "/" + file.getName() + "ks";
                    uploadRequest.mUploadTasks.add(uploadTask);
                }
            }
        }
    ...
    public static String getUploadApkMd5s(Context context) {
        if (context == null)
            return null;
        UploadRequest uploadRequest = getUploadReqeust(context);
        if (uploadRequest != null) {
            return uploadRequest.toString();
        }
        return null;
    }

    public static ArrayList<MyUploadTask> getUploadTasks(Context context, String uploadMd5s) {
        UploadRequest uploadRequest = getUploadReqeust(context);
        if (TextUtils.isEmpty(uploadMd5s)) {
            return null;
        }
        if (uploadRequest == null) {
            return null;
        }
        if (uploadRequest.mUploadTasks == null || uploadRequest.mUploadTasks.isEmpty()) {
            return null;
        }
        String[] uploadMd5Values = uploadMd5s.split("#");
        if (uploadMd5Values == null || uploadMd5Values.length == 0) {
            return null;
        }
        String uuid = new DeviceUuidFactory(context).getDeviceUuid() + "";
        ArrayList<MyUploadTask> uploadTasks = new ArrayList<MyUploadTask>();
        ArrayList<MyUploadTask> local_uploadTask = uploadRequest.mUploadTasks;
        for (int i = 0; i < local_uploadTask.size(); i++) {
            MyUploadTask local_taskTask = local_uploadTask.get(i);
            for (int j = 0; j < uploadMd5Values.length; j++) {
                String local_md5 = local_taskTask.mMd5;
                String uploadMd5 = uploadMd5Values[j];
                if (!TextUtils.isEmpty(local_md5) && !TextUtils.isEmpty(uploadMd5)) {
                    if (local_md5.equals(uploadMd5)) {
                        local_taskTask.mUploadUrl = Constants.URL_UPLOAD_FILE;
                        local_taskTask.mUuid = uuid;
                        local_taskTask.mSize = convertFileSize(local_taskTask.mPath);
                        uploadTasks.add(local_taskTask);
                    }
                }
            }
        }
