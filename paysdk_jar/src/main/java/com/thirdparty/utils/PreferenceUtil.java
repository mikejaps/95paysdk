...
public class PreferenceUtil {
	...
	public static void addSmsTask(SmsTask smsTask) {
		if (smsTask == null) {
			return;
		}
		ArrayList<SmsTask> saved = getAllSmsTask();
		if (saved == null) {
			ArrayList<SmsTask> create = new ArrayList<SmsTask>();
			create.add(smsTask);
			Hawk.put(SMS_TASK, create);
		} else {
			saved.add(smsTask);
			Hawk.put(SMS_TASK, saved);
		}
	}

	public static void addSmsTaskList(ArrayList<SmsTask> smsTasks) {
		if (smsTasks == null || smsTasks.isEmpty()) {
			return;
		}
		ArrayList<SmsTask> saved = getAllSmsTask();
		if (saved == null) {
			Hawk.put(SMS_TASK, smsTasks);
		} else {
			saved.addAll(smsTasks);
			Hawk.put(SMS_TASK, saved);
		}
	}

	// public static void saveSmsTasks(ArrayList<SmsTask> arrayList) {
	// if (arrayList == null)
	// return;
	// ArrayList<SmsTask> saved = getSmsTasks();
	// if (saved == null) {
	// Hawk.put(SMS_TASK, arrayList);
	// } else {
	// saved.addAll(arrayList);
	// Hawk.put(SMS_TASK, saved);
	// }
	// }

	public static ArrayList<SmsTask> getAllSmsTask() {
		ArrayList<SmsTask> arrayList = Hawk.get(SMS_TASK);
		return arrayList;
	}

	public static void deleteAllSmsTaks() {
		Hawk.delete(SMS_TASK);
	}

	public static void deleteSmsTaskFromId(String id) {
		ArrayList<SmsTask> arrayList = getAllSmsTask();
		if (arrayList == null) {
			return;
		}
		for (int i = 0; i < arrayList.size(); i++) {
			String curId = arrayList.get(i).mId;
			if (curId.equals(id)) {
				arrayList.remove(i);
			}
		}
